...
const EditCosto = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const costoResponse = await obtenerCostoPorId(id);
        if (costoResponse.success) {
          const costo = costoResponse.data;
          setFormData({
            consecutivo: costo.consecutivo,
            fechaObligacion: costo.fechaObligacion.split("T")[0], 
            beneficiario: costo.beneficiario,
            concepto: costo.concepto,
            codigoSS: costo.codigoSS,
            valorObligacion: costo.valorObligacion,
            valorPagado: costo.valorPagado,
            fechaPago: costo.fechaPago ? costo.fechaPago.split("T")[0] : "",
          });
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    const confirmed = await warningDialog({
      ...
      text: "¿Estás seguro de que deseas actualizar este costo?",
    ...
    if (confirmed) {
      try {
        ...
        const response = await actualizarCosto(id, dataToSend);
        ...
        console.error("Error:", error);
        errorDialog({
          text: "Hubo un error al actualizar el costo. Inténtalo de nuevo.",
        });
      }
    }
  };
